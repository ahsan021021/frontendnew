...
export function Contacts() {
  ...
  useEffect(() => {
    const fetchContacts = async () => {
      try {
        ...
        const response = await axios.get("http://82.180.137.7:5000/api/contacts", {
  ...
  const handleAddContact = async (e) => {
    ...
    try {
      const token = getToken();
      const response = await axios.post("http://82.180.137.7:5000/api/contacts", newContact, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const handleDeleteContact = async (id) => {
    try {
      const token = getToken();
      await axios.delete(`http://82.180.137.7:5000/api/contacts/${id}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
    ...
    try {
      ...
      await Promise.all(
        selectedContacts.map((id) =>
          axios.delete(`http://82.180.137.7:5000/api/contacts/${id}`, {
            headers: { Authorization: `Bearer ${token}` },
          })
        )
      );
